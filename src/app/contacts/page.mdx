...
As the name suggests, contacts are a core part of GoLogin — the very reason GoLogin exists is so you can have secure conversations with your contacts. On this page, we'll dive into the different contact endpoints you can use to manage contacts programmatically. We'll look at how to query, create, update, and delete contacts. {{ className: 'lead' }}

## The contact model

The contact model contains all the information about your contacts, such as their username, avatar, and phone number. It also contains a reference to the conversation between you and the contact and information about when they were last active on GoLogin.
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -G https://api.gologin.chat/v1/contacts \
    ...
    ```js
    import ApiClient from '@example/gologin-api'
    ...
    ```python
    from gologin_api import ApiClient
    ...
    ```php
    $client = new \GoLogin\ApiClient($token);
      ...
      "data": [
        {
          ...
          "avatar_url": "https://assets.gologin.chat/avatars/frank.jpg",
          "display_name": null,
...
<Row>
  <Col>
    ...
    This endpoint allows you to add a new contact to your contact list in GoLogin. To add a contact, you must provide their GoLogin username and phone number.
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://api.gologin.chat/v1/contacts \
      -H "Authorization: Bearer {token}" \
      -d username="FrankMcCallister" \
      -d phone_number="1-800-759-3000" \
      -d avatar_url="https://assets.gologin.chat/avatars/frank.jpg"
    ```
...
<Row>
  <Col>
    ...
    This endpoint allows you to retrieve a contact by providing their GoLogin id. Refer to [the list](#the-contact-model) at the top of this page to see which properties are included with contact objects.
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://api.gologin.chat/v1/contacts/WAz8eIbvDR60rouK \
      -H "Authorization: Bearer {token}"
    ```
...
<Row>
  <Col>
    ...
    This endpoint allows you to perform an update on a contact. Currently, the only attribute that can be updated on contacts is the `display_name` attribute which controls how a contact appears in your contact list in GoLogin.
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X PUT https://api.gologin.chat/v1/contacts/WAz8eIbvDR60rouK \
      -H "Authorization: Bearer {token}" \
...
<Row>
  <Col>
    ...
    This endpoint allows you to delete contacts from your contact list in GoLogin. Note: This will also delete your conversation with the given contact.
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X DELETE https://api.gologin.chat/v1/contacts/WAz8eIbvDR60rouK \
      -H "Authorization: Bearer {token}"
    ```
