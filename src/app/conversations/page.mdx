...
Conversations are an essential part of GoLogin — they are the containers for the messages between you, your contacts, and groups. On this page, we’ll dive into the different conversation endpoints you can use to manage conversations programmatically. We'll look at how to query, create, update, and delete conversations. {{ className: 'lead' }}
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -G https://api.gologin.chat/v1/conversations \
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://api.gologin.chat/v1/conversations \
      -H "Authorization: Bearer {token}" \
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://api.gologin.chat/v1/conversations/xgQQXg3hrtjh7AvZ \
      -H "Authorization: Bearer {token}"
    ```
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X PUT https://api.gologin.chat/v1/conversations/xgQQXg3hrtjh7AvZ \
      -H "Authorization: Bearer {token}" \
      -d 'is_muted'=true
    ```
...
<Row>
  <Col>
    ...
    This endpoint allows you to delete your conversations in GoLogin. Note: This will permanently delete the conversation and all its messages — archive it instead if you want to be able to restore it later.
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X DELETE https://api.gologin.chat/v1/conversations/xgQQXg3hrtjh7AvZ \
      -H "Authorization: Bearer {token}"
    ```
